import {
  ...
  url,
  permissao,
} from '../../src/class/comum/evolucaoponto_comum_criptografia.js';
import {
  TPonto,
  PontoRoot,
} from '../../src/class/base/evolucaoponto_base_ponto.js';
import {
  UsuarioRoot,
  TUsuario,
} from '../../src/class/base/evolucaoponto_base_usuario.js';
...
async function chamarBatida() {
  ...
  const coIGUAL = 1;
  const ftInteger = 3;
  const dtSemConverter = document
    .querySelector('.dataCompleta')
    .textContent.trim();
...
function calcularHoras(arrayHoras) {
  ...
  for (let i = 0; i < arrayHoras.length; i += 2) {
    ...
    if (exit) {
      // Garantir que haja um par de saída
  ...
  minTrabFormatadas =
    Math.abs(remainingMinutes) < 10
      ? remainingMinutes < 0
        ? '0' + Math.abs(remainingMinutes * -1)
        : '0' + remainingMinutes
      : remainingMinutes;

  DiferencaHrsTrabFormatadas =
    Math.abs(horasInteiras) < 10
      ? horasInteiras < 0
        ? '0' + Math.abs(horasInteiras * -1)
        : '0' + horasInteiras
      : horasInteiras;
  ...
  SomaHrsTrabFormatadas =
    Math.abs(horasTrabalhados) < 10
      ? horasTrabalhados < 0
        ? '0' + Math.abs(horasTrabalhados * -1)
        : '0' + horasTrabalhados
      : horasTrabalhados;

  let minutosFormatados =
    Math.abs(minutosTrabalhados) < 10
      ? minutosTrabalhados < 0
        ? '-0' + Math.abs(minutosTrabalhados * -1)
        : '0' + minutosTrabalhados
      : minutosTrabalhados;
  ...
  document.getElementById(
    'soma'
  ).innerText = `${SomaHrsTrabFormatadas}:${minutosFormatados}`;
...
function pontosDiaSelecionado(diaSelecionado, mêsSelecionado, anoSelecionado) {
  ...
  diaFormatado =
    Math.abs(diaSelecionado) < 10
      ? diaSelecionado < 0
        ? '0' + Math.abs(diaSelecionado)
        : '0' + diaSelecionado
      : diaSelecionado;

  mêsSelecionadoFormatado =
    Math.abs(mêsSelecionado) < 10
      ? mêsSelecionado < 0
        ? '0' + Math.abs(mêsSelecionado)
        : '0' + mêsSelecionado
      : mêsSelecionado;

  const dataCompleta =
    diaFormatado + '/' + mêsSelecionadoFormatado + '/' + anoSelecionado;
